...
class SubmitRecipe extends Component {
  ...
  submitRecipeHandler = () => {
    ...
    if (formIsValid) {
      ...
      axios
        .post("https://foodrecipejson.firebaseio.com/RecipeList.json", postData)
        .then((response) => {
          console.log(response);
          if (response.status == 200) {
            this.setState({
              success:
                "Successfully Submitted! Click Home to view the recipe added",
            });
            window.scrollTo({ top: 0, left: 0, behavior: "smooth" });
          }
        });
    ...
    return (
      <div>
        ...
        <SubmitRecipeBody>
          ...
          <SubmitButton onClick={this.submitRecipeHandler}>
            Submit Recipe
          </SubmitButton>
        </SubmitRecipeBody>
      </div>
